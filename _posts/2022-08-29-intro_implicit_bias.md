...
Concretely, say you made a fantastic new second order optimization algorithm which optimizes the loss function 10x faster than standard first order methods. Keep in mind that the deep learning models were developed and compared under the implicit bias of first order optimization methods. Chances are that your second order optimizer significantly changes that implicit bias, giving worse final model performance than the implicit bias the model was developed under. 

Let's say we found a way to better characerize the implicit bias of modern deep learning models. Maybe we could improve it. Maybe we could train models faster, without losing out on performance. Maybe we could get rid of tens of annoying hyperparameters. I want to find out.
